...
class Jumin {
	...
	/*
	 * int size = juminno.length(); int sum = 0; int multi = 2; //2,3,4,5... 곱할때
	 * 쓰이는것. for(int i=0; i<size-1; i++) { if(multi > 9) { multi = multi - 8; sum =
	 * sum + multi*((int) juminno.charAt(i) - 48); multi = multi + 1; } else { sum =
	 * sum + multi*((int) juminno.charAt(i) - 48); multi = multi + 1; } } String mn
	 * = juminno.substring(size-1,size); int ms = (11 - sum%11)%10;
	 * if(Integer.parseInt(mn) == ms) { flag = true; }
	 */
	...
	public void disp() {
		// 문)생년월일, 성별, 나이, 띠 출력하기
		/*
		 * 생년월일 : 1989년 12월 30일 성별 : 남자 나이 : 34 -> GregorianCalendar클래스 이용 띠 : -> 태어난%12
		 * (0원숭이 1닭 2개 ~~ 11양)
		 */
