...
public class test01_wrapper {
	...
	/*
	 * String st = ""; for(int i=0; i<str.length();i++) { char d = str.charAt(i);
	 * if(d>='A' && d<='Z') { d=Character.toLowerCase(d); st = st + d; } else if
	 * (d>='a' && d<='z') { d=Character.toUpperCase(d); st = st + d; } else st = st
	 * + d; } System.out.println(st);
	 */
